// src/pages/existing/RetestUnit.tsx
import React, { useState } from "react";
import {
  IonPage, IonHeader, IonToolbar, IonTitle, IonContent,
  IonButtons, IonBackButton, IonCard, IonCardHeader, IonCardTitle,
  IonCardContent, IonList, IonItem, IonLabel, IonBadge, IonButton, useIonToast
} from "@ionic/react";
...
const RetestUnit: React.FC = () => {
  ...
  const runTest = async () => {
    setPower("pass"); setIgn("pass"); setGps("fail"); setCell("pass");
  ...
  return (
    <IonPage>
      ...
      <IonContent className="ion-padding">
        <IonCard>
          <IonCardHeader><IonCardTitle>Live Checks</IonCardTitle></IonCardHeader>
          <IonCardContent>
            <IonList>
              <IonItem><IonLabel>Power</IonLabel>{badge(power)}</IonItem>
              <IonItem><IonLabel>Ignition</IonLabel>{badge(ign)}</IonItem>
              <IonItem><IonLabel>GPS</IonLabel>{badge(gps)}</IonItem>
              <IonItem><IonLabel>Cellular</IonLabel>{badge(cell)}</IonItem>
            </IonList>
          </IonCardContent>
        </IonCard>
        <IonButton expand="block" onClick={runTest}>Run Test</IonButton>
      </IonContent>
    </IonPage>
  );
};
